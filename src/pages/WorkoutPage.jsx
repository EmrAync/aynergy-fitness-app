...
const WorkoutPage = () => {
    ...
    const handleCompleteWorkout = async (planId) => {
        if (!currentUser || !planId) return;

        // Kaydedilecek planın güncel halini state'den buluyoruz.
        const completedPlan = workoutPlans.find(p => p.id === planId);
        if (!completedPlan) {
            notifyError("Could not find the plan to save.");
            return;
        }

        try {
            // 1. Yeni bir "workoutLogs" koleksiyonuna tüm antrenman detayını kaydediyoruz.
            await addDoc(collection(db, `users/${currentUser.uid}/workoutLogs`), {
                ...completedPlan,
                completedAt: new Date(),
            });

            // 2. Kullanıcının profiline son antrenman tarihini ve seriyi güncellemeye devam ediyoruz.
            const profileRef = doc(db, `users/${currentUser.uid}/profile`, "data");
            // Not: Streak (seri) mantığını daha da geliştirmek için buraya ek mantık eklenebilir.
            // Şimdilik sadece son tarihi güncelliyoruz.
            await updateDoc(profileRef, {
                lastWorkoutDate: new Date()
            });

            notifySuccess(t('workoutCompletedSuccess')); // Dil dosyasına 'workoutCompletedSuccess' eklenmeli

            // Antrenman bittikten sonra setleri sıfırlamak iyi bir kullanıcı deneyimi olabilir.
            // Bu kısım şimdilik opsiyoneldir.

        } catch (error) {
            console.error("Error completing workout:", error);
            notifyError(t('workoutCompletedError')); // Dil dosyasına 'workoutCompletedError' eklenmeli
        }
    };
