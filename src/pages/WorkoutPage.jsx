...
const WorkoutPage = () => {
  ...
  const handleCreatePlan = async () => {
    ...
    if (
      ...
      workoutPlans.length >= MAX_FREE_PLANS
    ) {
  ...
  const handleCompleteWorkout = async (planId) => {
    ...
    try {
      // 1. Özel log'a kaydetmeye devam et
      await addDoc(collection(db, `users/${currentUser.uid}/workoutLogs`), {
        ...completedPlan,
        completedAt: new Date(),
      });

      // 2. YENİ: Herkesin görebileceği genel bir aktivite kaydı oluştur
      await addDoc(collection(db, "activities"), {
        userId: currentUser.uid,
        userName: userProfile.name || "Someone",
        activityType: "workout_completed",
        workoutName: completedPlan.name,
        exerciseCount: completedPlan.exercises.length,
        createdAt: new Date(),
      });

      // 3. Profili ve seriyi güncelle
      const profileRef = doc(db, `users/${currentUser.uid}/profile`, "data");
      await updateDoc(profileRef, {
        lastWorkoutDate: new Date(),
      });

      notifySuccess(t("workoutCompletedSuccess"));
    } catch (error) {
      console.error("Error completing workout:", error);
      notifyError(t("workoutCompletedError"));
    }
  };

  const selectedPlanDetails = workoutPlans.find(
      ...
      <div className="lg:col-span-1 space-y-6">
        <Card>
          ...
          <div className="flex space-x-2 mb-4">
            ...
            />
            <Button onClick={handleCreatePlan} className="w-auto px-4 py-2">
              {t("createPlan")}
            </Button>
          </div>
          <WorkoutPlanList
        ...
        <Button
          ...
          className="bg-green-600 hover:bg-green-700 w-full flex items-center justify-center space-x-2 text-lg py-3"
        >
