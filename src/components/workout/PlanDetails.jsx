  ...
  return (
    <Card>
      <div className="flex justify-between items-center mb-4">
        ...
        <Button onClick={() => onDeletePlan(selectedPlan.id)} className="w-auto px-4 py-1 text-sm bg-red-600 hover:bg-red-700">
          {t('deletePlan')}
        </Button>
      </div>
      <h4 className="font-semibold mb-2">{t('exercisesInPlan')}:</h4>
      <ul className="space-y-4">
        {selectedPlan.exercises.map((exercise, index) => (
          <li key={index} className="bg-gray-50 p-4 rounded-md">
            <div className="flex items-center space-x-4 mb-2">
              <div className="w-10 h-10 flex-shrink-0">
                <img src={exercise.gif} alt={exercise.name} className="w-full h-full object-cover rounded-md" />
              </div>
            ...
            <div className="pl-14">
              <h5 className="text-sm font-semibold text-gray-600 mb-1">{t('set')}s:</h5>
